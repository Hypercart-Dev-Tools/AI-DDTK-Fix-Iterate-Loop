...
- [Operator Preflight](#operator-preflight)
...
- [Redaction Intake and Naming Hygiene](#redaction-intake-and-naming-hygiene)
- [Portable Script](#portable-script)
...
- [x] **Phase 0 — Inventory and Classification**
- [x] **Phase 1 — Canonical Structure and Retention Rules**
- [x] **Phase 2 — Build a Repo Metadata Catalog**
...
Before starting any cleanup pass in a real repo, protect the current state first.

- Make a commit for the current state, or create a new branch for the cleanup work.
- Do this before renames, scrubbing, promotion moves, or large archival passes.
- If the repo is already dirty, either commit the current work first or create a dedicated branch that clearly isolates the cleanup changes.
- Treat this as the operator checkpoint before the cleanup workflow begins, not as an optional afterthought.
...
- [x] A short policy section or reference doc update that names the lifecycle rules for canonical, experimental, generated, and temporary files. See `PROJECT/DOCS-INSTRUCTIONS.md`.
...
- [x] Add a rule for how manual overrides are stored when auto-detection is wrong (`tools/repo-catalog.overrides.json`).
...
- [x] Decide where the catalog lives and whether it is checked in or regenerated (`tools/mcp-server/repo-catalog.json`, regenerated by `./PROJECT/cleanup.sh catalog`; `tools/cleanup.sh` remains a compatibility wrapper).

### Deliverable

- [x] A machine-readable catalog that can drive cleanup reports, folder summaries, and later search indexing. See `tools/mcp-server/repo-catalog.json`.
...
When deciding whether a doc or file should stay active, move to `3-DONE`, move to `4-MISC`, or remain in place, use evidence in this order rather than folder name or recency alone.

1. Check the current document state.
	Read the frontmatter `status`, checklist completion, and whether the body still describes unfinished work versus a completed or reference-only artifact.
2. Check `CHANGELOG.md` for completion signals.
	If the capability or plan described by the doc is already recorded as shipped, completed, promoted, or replaced in `CHANGELOG.md`, that is a strong signal that the doc is no longer active working state.
3. Check `4X4.md` for active priority signals.
	If the work is still represented as a current objective in `4X4.md`, that is a strong signal to keep it in `1-INBOX` or `2-WORKING`. If it is absent while the changelog says it shipped, that favors `3-DONE` or `4-MISC`.
4. Check git commit history for freshness and type of change.
	Look at the most recent commits touching the file. Recent substantive implementation-aligned commits can support `working`; old commits or doc-only maintenance edits without active execution are weak evidence by themselves.
5. Reconcile conflicts explicitly.
	If `CHANGELOG.md` says shipped but the doc checklist is still open, update the doc to match reality before moving it. If the doc says done but `4X4.md` still treats it as active, keep it active until the planning surfaces are aligned.

### Decision Rules

- Move to `3-DONE` when the described work is implemented or completed and the file primarily serves as completion history.
- Move to `4-MISC` when the file is reference material, brainstorming, paused indefinitely, superseded planning, or third-party analysis that is no longer driving active execution.
- Keep in `2-WORKING` when `4X4.md`, checklist state, and recent implementation history all indicate active execution.
- Keep in `1-INBOX` when the idea is still proposed work and there is not yet strong evidence of active execution.
- Do not move a file to `done` or `misc` purely because it is old. Age is only a supporting signal.
...
## Redaction Intake and Naming Hygiene

Purpose: prevent real client and project identifiers from lingering in repo docs, temp artifacts, filenames, and publication-bound materials.

### Scope

- Redact explicit client names and project names when they are not required as durable source-of-truth data.
- Flag filenames that contain client or project identifiers, not just file contents.
- Treat hostnames, branded environment names, export filenames, and dump filenames as likely identifier carriers.
- Prefer neutral placeholders such as `[CLIENT]`, `[PROJECT]`, and `[CLIENT-DOMAIN]` in docs meant for reuse, sharing, or publication.

### Agent Intake Rule

Before running a broad cleanup, doc-packaging, or repo-sharing pass, the orchestrating AI agent should ask the user for the terms that need protection.

Prompt the user for:

- known client names
- known project or product names
- domains, subdomains, and hostnames tied to the client
- filenames or directory names that include those terms
- preferred replacement tokens if they want something other than the default placeholders

If the user does not provide a list, the agent should still run a best-effort scan for likely proper nouns and branded filenames, but treat that result as a review queue rather than auto-apply truth.

### Reusable Prompt Template

Orchestrating agents can reuse this prompt before running scrub-related work:
...
Notes:
- Include alternate spellings, acronyms, and lowercase or slug variants if they matter.
...
The same intake can be emitted from the helper command `./PROJECT/cleanup.sh scrub-intake` or `./PROJECT/cleanup.sh scrub-intake --json`.

### Execution Rule

- Use the scrub workflow for document content replacements.
- Review filenames separately because a safe content replacement does not rename the file itself.
- Require explicit review before renaming files that may be referenced by code, links, or external systems.
- Keep the real term list outside git in the scrub-list input, never in a tracked planning doc.

### Exit Criteria

- Cleanup and publication passes include a deliberate identifier-redaction review, not just content cleanup.
- The orchestrating agent has an explicit intake step for protected names before applying redaction.
- Client-specific filenames are either renamed safely or left in place with a documented reason.
...
<!-- PORTABLE-SCRIPT:SECTION-START -->
```bash
#!/usr/bin/env bash
# =============================================================================
# cleanup.sh — PROJECT folder hygiene · catalog + Phases 0–4 + repo-case + secrets + scrub
# version 1.9 - attn: update this number as improvements are added
# =============================================================================
# Catalog (catalog): build the repo metadata catalog under tools/mcp-server/.
...
#
# USAGE — Catalog (repo metadata registry)
#   ./PROJECT/cleanup.sh catalog           # regenerate tools/mcp-server/repo-catalog.json
#   ./PROJECT/cleanup.sh catalog --json    # write catalog and emit a JSON summary
#
...
#
# PHASE ROADMAP
#   Catalog (this) — repo metadata catalog: build tools/mcp-server/repo-catalog.json
...
COMMAND="hygiene"       # catalog | hygiene (Phase 1) | scan (Phase 2) | meta (Phase 3) | promote (Phase 4) | uppercase | secrets | scrub-intake | scrub | portable-doc
...
CATALOG_PATH="$REPO_BASE/tools/mcp-server/repo-catalog.json"
OVERRIDES_PATH="$REPO_BASE/tools/repo-catalog.overrides.json"
...
run_catalog() {
  ...
  $JSON_MODE || echo "🔍 Phase 0: Building repository metadata catalog..."

  mkdir -p "$(dirname "$CATALOG_PATH")"

  python3 - "$REPO_BASE" "$CATALOG_PATH" "$OVERRIDES_PATH" <<'PYEOF'
import json
import os
import subprocess
import sys
...
def load_overrides(path):
    if not os.path.exists(path):
        with open(path, "w", encoding="utf-8") as handle:
            json.dump({
                "_README": "Path-keyed overrides for cleanup catalog generation. Each key is a tracked repo path. Values may override lifecycle_class, owner_tool, canonical, generated, status, notes, and tags.",
                "overrides": {}
            }, handle, indent=2)
        return {}
...
def get_notes(path, lifecycle_class, owner_tool):
    ...
    if owner_tool in {"project-cleanup", "repo-hygiene"}:
        notes.append("Source of truth for generating the repo metadata catalog.")
    if owner_tool == "repo-cleanup":
        notes.append("Compatibility wrapper for PROJECT/cleanup.sh catalog.")
    return " ".join(notes)
...
PYEOF
  ...
  catalog_count="$(python3 - "$CATALOG_PATH" <<'PYEOF'
import json
import sys
...
PYEOF
  ...
  if $JSON_MODE; then
    ...
    printf '✓ Catalog built with %s tracked files.\n' "$catalog_count"
...
- [ ] Should repo cleanup checks become part of `preflight.sh`, `post-flight`, or a separate maintenance command?
