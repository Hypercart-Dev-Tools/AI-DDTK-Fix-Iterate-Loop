# Run 3 — paste-ready prompt for Codex

Open the Codex session on the **shared** `experiment/coordination-layer` working
...
Your identity is the `--agent codex` flag you pass to every `tick` command —
...
1. `experimental/coordination-layer/bin/tick take --agent codex`
