# Run 2 — paste-ready prompt for Codex

Open the Codex session in its own clone of the `experiment/coordination-layer`
...
You are **codex**, one of two AI agents building a small app concurrently on a
shared git branch, coordinated through the `tick` CLI. The other agent (gemini)
...
1. `experiments/coordination-layer/bin/tick next --agent codex`
   — tells you which task is yours.
2. `experiments/coordination-layer/bin/tick claim <TASK-ID> --agent codex --paths "<declared scope>"`
...
6. `experiments/coordination-layer/bin/tick done <TASK-ID> --agent codex`
7. Go back to step 1.
...
- **If you get genuinely stuck** on a task (tests won't pass after real
  effort), run
  `experiments/coordination-layer/bin/tick break <TASK-ID> --agent codex --reason "..."`
