...
# Coordinator reaps alice's claims (run here from B's clone).
git -C "$B" pull -q --rebase origin main
TICK_TS=2026-05-04T10:00:05.000Z tick_in "$B" reap alice --by coordinator >"$WORK/reap.out"
...
tick_in "$B" project >/dev/null
if grep -qE "^- TASK-1 by alice" "$B/.tick/STATE.md"; then
...
TICK_TS=2026-05-04T10:00:10.000Z tick_in "$B" claim TASK-1 --agent bob --paths "src/one/**" >"$WORK/bob.out"
...
TICK_TS=2026-05-04T10:00:11.000Z tick_in "$B" reap alice --by coordinator >"$WORK/reap2.out"
